...
#'   by using their superimposed configurations (from within \code{relw}),
#'   calculates the shape difference between them, and plots such differences
#'   on provided meshes (\code{refsur}).
#' @usage shapeDiff(x,relw,refsur,refmat,mshape_sur = NULL,
...
#'   on the consensus configuration derived from \code{relw}.
...
#'                    relw = pca,refsur = list(sur_pan,sur_alo),
#'                    refmat = list(ldm_pan,ldm_alo))
#'   }

shapeDiff<-function(x,relw,refsur,refmat,mshape_sur=NULL,
                    pal=NULL,NAcol="gray90",show.names=TRUE){
